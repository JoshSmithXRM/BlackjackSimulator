namespace Blackjack.Services
{
    public class GameRound : IGameRound
    {
        ...
        public List<HandResult> Play(int numberOfPlayerHands, decimal betAmount, bool simulationMode = false)
        {
            ...
            {
                var result = _handEvaluator.EvaluateHand(hand, _dealerHand);
        ...
        private void InitializeHands(int numberOfPlayerHands, decimal betAmount)
        {
            _dealerHand = _handFactory.CreateHand();
            _playerHands.Clear();
            for (var i = 0; i < numberOfPlayerHands; i++)
            {
        ...
        private void PlayHand(IHand playerHand, IHand dealerHand)
        {
            while (true)
            {
                if (_simulationMode)
                {
                    if (playerHand.IsBust || playerHand.IsBlackjack || playerHand.GetTotal() == 21)
                    {
                        return;
                    }

                    var recommendation = _cardCountingService.GetRecommendation(playerHand, dealerHand.GetUpCard());
                ...
                else
                {
                    ...
                    var playerAction = _gameInputService.GetPlayerAction(validActions);
